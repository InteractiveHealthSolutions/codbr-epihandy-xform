...
public class TransportLayer implements Runnable, AlertMessageListener {
	...
	protected synchronized void handleRequest() {
		...
		try{
			...
			case CON_TYPE_NULL:
				...
				getUserSettings(this.display,this.prevScreen,userName,password, null);
				break;
			default:
				break;
			}
	...
	private void readResponseData(DataInputStream dis) throws Exception {
		...
		try {
			...
			if (status == ResponseHeader.STATUS_SUCCESS) {
				...
				else if (status == ResponseHeader.STATUS_INVALID_SESSION_REFERENCE) {
					s = MenuText.INVALID_SESSION_REFERENCE();
				}
	...
	protected void connectHttp() throws IOException{
		...
		try{
			...
			if(status != HttpConnection.HTTP_OK){
				if (status == HttpConnection.HTTP_NOT_FOUND) {
					// error shown when the requested resource is unavailable. for example when the servlet name is wrong.
					this.eventListener.errorOccured(MenuText.ERROR() + ". " + MenuText.SERVER_ADDRESS_PROBLEM() + " (" + MenuText.ERROR_CODE() + " " +status + ")", null);
				} 
				conType = CON_TYPE_NULL; // reset connection type
			} else {
			...
			e.printStackTrace();
			// error could be due to a wrong port,hostname or even the server is down.
			this.eventListener.errorOccured(MenuText.ERROR() + ". " + MenuText.SERVERDOWN_PROBLEM(),e);
	 ...
	 */
	public void getUserSettings(Display display, Displayable prevScreen, String name, String password, ConnectionSettingsListener listener){
		this.prevScreen = prevScreen;
		conSettings.setDisplay(display);
		conSettings.setPrevScreen(prevScreen);
		conSettings.setTitle(MenuText.CONNECTION_SETTINGS());
		conSettings.getUserSettings(display, prevScreen,conType,conTypes,conParams,this,name,password,connectionParameters, listener);
	}
}
