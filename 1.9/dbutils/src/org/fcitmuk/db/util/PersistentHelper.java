...
public class PersistentHelper {
	...
	/**
	 * Writes a vector (of short size) of persistent objects from a stream.
	 * 
	 * @param persistentVector
	 * @param dos
	 * @throws IOException
	 */
	public static void writeMedium(Vector persistentVector, DataOutputStream dos) throws IOException {	
		if(persistentVector != null){
			dos.writeShort(persistentVector.size());
			for(int i=0; i<persistentVector.size(); i++ ){
				((Persistent)persistentVector.elementAt(i)).write(dos);
			}
		}
	...
	/**
	 * Reads a vector (with short size) of a persistent class from stream.
	 * 
	 * @param dis
	 * @param cls
	 * @return
	 ...
	 * @throws IllegalAccessException
	 */
	public static Vector readMedium(DataInputStream dis, Class cls) throws IOException, InstantiationException,IllegalAccessException {
	...
	*/
	public static Hashtable read(DataInputStream dis) throws IOException {
		...
		for(byte i=0; i<len; i++ )
			stringHashtable.put(dis.readUTF().intern(), dis.readUTF().intern());
