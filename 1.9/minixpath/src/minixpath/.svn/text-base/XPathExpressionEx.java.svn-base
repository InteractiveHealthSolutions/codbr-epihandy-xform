 ...
 * 
 */
public class XPathExpressionEx 
{
	String[] locationStepStringsArray;
	XPathLocationStepEx[] locationStepArray;
	Vector resultNodeSet;
	String expression = null;
	Node startNode = null;
		
	public XPathExpressionEx (Node startNode, String expression)
	{
		...
		if(expression.startsWith("//")) {
			...
			tmp.addElement("//");
			expression = new String(expression.toCharArray(), 2, expression.length()-2).intern();
		} else if(expression.startsWith("/")) {
			tmp.addElement("/");
			//trace the root element
			expression = new String(expression.toCharArray(), 1, expression.length()-1).intern();
		} 
		...
		for(int start = 0, end = 0; end < expression.length()-1 && end!=-1; start = end+1) {
			...
			tmp.addElement(new String(expression.toCharArray(), start, 
				(end!=-1?end:expression.length())-start).intern());
		}
		...
		for(int j=0; j < locationStepStringsArray.length; j++)
		{
			...
			XPathLocationStepEx locationStep = new XPathLocationStepEx(locationStepString);
