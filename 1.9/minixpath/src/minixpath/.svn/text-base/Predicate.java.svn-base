...
public class Predicate 
{
	...
	Predicate(Vector inNodeSet, String predicateExpr)
	{
		...
		Member member1 = new Member(new String(predicateExpr.toCharArray(), 0, index).intern());
		Member member2 = new Member(new String(predicateExpr.toCharArray(), index+1, predicateExpr.length()-index-1).intern());
	...
	class Member 
	{
		...
		Member(String op)
		{
			...
			if(op.startsWith("@")) {
				attribute = new String(op.toCharArray(), 1, op.length()-1).intern();
			}
