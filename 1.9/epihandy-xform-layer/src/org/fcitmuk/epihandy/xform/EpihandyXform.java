...
public class EpihandyXform{
	...
	public static FormData fromXMLToFormData(String formDefXml, String formDataXml) {
		FormDef formDef = getFormDef(getDocument(new StringReader(formDefXml)));
		FormData formData = new FormData(formDef);
		Document dataDoc = getDocument(new StringReader(formDataXml));
		Element dataEl = dataDoc.getRootElement();
		for (int i=0; i<formData.getPages().size(); i++) {
			PageData pageData = (PageData)formData.getPages().elementAt(i);
			for (int j=0; j<pageData.getQuestions().size(); j++) {
				QuestionData questionData = (QuestionData)pageData.getQuestions().elementAt(j);
				if (questionData.getDef().getType() != QuestionDef.QTN_TYPE_REPEAT) {
					String answer = getAnswerText(dataEl, questionData.getDef().getVariableName());
					if (answer != null) {
						formData.setValue(questionData.getDef().getVariableName(), answer);
					}
				}
				else {
					setRepeatFormDataValues(dataEl,questionData,formData);
				}
	...
	private static String getAnswerText(Element rootElement, String xpath) {
		XPathExpression xpls = new XPathExpression(rootElement, xpath);
		Vector result = xpls.getResult();
		for (Enumeration e = result.elements(); e.hasMoreElements();) {
			Object obj = e.nextElement();
			if (obj instanceof Element) {
				String value = getTextContent((Element)obj);
				return value;
			}
		}
		return null;
	...
	private static void setRepeatFormDataValues(Element rootElement, QuestionData qtnData, FormData formData) {
		RepeatQtnsDef repeatQtnDef = qtnData.getDef().getRepeatQtnsDef();
		RepeatQtnsDataList answer = new RepeatQtnsDataList();
		qtnData.setAnswer(answer);
		...
		for (Enumeration e = result.elements(); e.hasMoreElements();) {
			...
			if (obj instanceof Element) {				
				Element elem = (Element) obj;
				RepeatQtnsData data = new RepeatQtnsData(counter++,repeatQtnDef);
				answer.addRepeatQtnsData(data);
				data.setQuestions(new Vector()); // clear default values
				for (int i=0; i<repeatQtnDef.getQuestions().size(); i++) {
					QuestionDef qDef = (QuestionDef)repeatQtnDef.getQuestions().elementAt(i);
					QuestionData qData = new QuestionData(qDef);
					if (qDef.getType() != QuestionDef.QTN_TYPE_REPEAT) {
						String repeatXpath = qDef.getVariableName().substring(qDef.getVariableName().lastIndexOf("/")+1);
						String answerText = getAnswerText(elem, repeatXpath);
						if (answer != null) {
							qData.setTextAnswer(answerText);
							data.addQuestion(qData);
						}
					}
					else {
						setRepeatFormDataValues(elem,qData,formData);
					}
				}
			}
		}
		...
		for (int i = 0; i < numOfEntries; i++) {
			if (!element.isText(i) && element.getType(i) == Element.ELEMENT) {
				...
				if(tagname.equals("submit"))
					...
					//MAIMOONA - ADDING REPEATS WAS NOT WORKING... REMOVING CONDITIONAL ADDING OF QUESTION
					addRepeatChildQtn(qtn,repeats,child,map,rptKidMap);
					if(child.getAttributeValue(null, "id") != null){
						map.put(child.getAttributeValue(null, "id"), qtn.getVariableName());
						formDef.addQuestion(qtn);
					}
					...
					//MAIMOONA : added variable IDs too in all actions and constraints, as code was changing question ids and refrences somewhere
					if(child.getAttributeValue(null, "relevant") != null){
						relevants.put(qtn.getVariableName(),child.getAttributeValue(null, "relevant"));
						if(child.getAttributeValue(null, "id") != null){
							relevants.put(child.getAttributeValue(null, "id"), child.getAttributeValue(null, "relevant"));
						}
						...
						actions.put(qtn.getVariableName(),action);
						if(child.getAttributeValue(null, "id") != null){
							actions.put(child.getAttributeValue(null, "id"), action);
						}
					}

					if(child.getAttributeValue(null,"constraint") != null){
						constraints.put(qtn.getVariableName(), child.getAttributeValue(null,"constraint"));
						if(child.getAttributeValue(null, "id") != null){
							constraints.put(child.getAttributeValue(null, "id"), child.getAttributeValue(null,"constraint"));
						}
						constraintMsgs.put(qtn.getVariableName(), child.getAttributeValue(null, "message"));
						if(child.getAttributeValue(null, "id") != null){
							constraintMsgs.put(child.getAttributeValue(null, "id"), child.getAttributeValue(null, "message"));
						}
					}
					...
					if(varName != null){
						...
						if(tagname.equals("select1") || tagname.equals("select")){
							...
							String nodeType = child.getAttributeValue(null, "type");
							if(nodeType == null){
								try{
									Element instanceNode = (Element)instanceDataNode.getParent();
									Element modelNode = (Element)instanceNode.getParent();
									XPathExpression xpls = new XPathExpression(modelNode, "bind[@id="+bind+"]");
									Vector result = xpls.getResult();
									if(result.size() > 0)
										nodeType = ((Element)result.get(0)).getAttributeValue(null,"type");
								}
								catch (Exception e) {
									e.printStackTrace();
								}
							...
							if(nodeType != null){
								setQuestionType(qtn,nodeType,child);
							}
						}
	...
	private static void addSkipRules(FormDef formDef, Hashtable map, Hashtable relevants, Hashtable actions){
		...
		while(keys.hasMoreElements()){
			String qtnid = (String)keys.nextElement();
			SkipRule skipRule = buildSkipRule(formDef, formDef.getQuestionId(qtnid), (String)relevants.get(qtnid), ++id, getAction(actions.get(qtnid)));
			if(skipRule != null)
				rules.add(skipRule);
		}
	...
	private static void addValidationRules(FormDef formDef, Hashtable map, Hashtable constraints, Hashtable constraintMsgs){
		...
		while(keys.hasNext()){
			String qtnid = (String)keys.next();
			ValidationRule validationRule = buildValidationRule(formDef, formDef.getQuestionId(qtnid),(String)constraints.get(qtnid),(String)constraintMsgs.get(qtnid));
