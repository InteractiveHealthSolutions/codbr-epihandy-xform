...
public class UserManager extends AbstractView implements AlertMessageListener {
	...
	public UserManager(Display display,Displayable prevScreen,String title,
			LogonListener logonListener, 
			DownloadUploadManager downloadMgr){
		super();
		setDisplay(display);
		setTitle(title);
		...
		setTransportLayerListener(downloadMgr.getTransportLayerListener());
	}
	...
	public void logOn(){
		...
		frm.addCommand(DefaultCommands.cmdExit);
	...
	private boolean isUserValid(String name, String password) {
		...
		if (failedLoginCount < 3) {
			// allow for the situation where the password has been changed on the server. if there are three or more
			// failed logins (in a row) then the user is downloaded again
			...
			failedLoginCount=0; // reset the failed login count
		}
		
		// retrieve the user from the server (this happens in the background)
		downloadMgr.setTransportLayerListener(transportLayerListener);
	...
	/**
	 * Processes the exit command event.
	 * 
	 * @param d - the screen object the command was issued for.
	 */
	private void handleExitCommand(Displayable d){
		if(logonListener != null)
