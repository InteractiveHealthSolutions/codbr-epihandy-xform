...
/** 
 * Handles display of forms. This class acts as a facade to the epihandy forms engine.
...
public class FormManager implements TransportLayerListener{
	...
	public FormManager(String title,Display display, FormListener formEventListener, Displayable currentScreen,
		...
		downloadMgr = new DownloadUploadManager(transportLayer, controller, title, this, true);
	...
	public void displayUserSettings(Display display, Displayable prevScreen){
		...
		userSettings.display(display, prevScreen, transportLayer,userMgr.getUserName(),userMgr.getPassword(), this.controller);
	}
