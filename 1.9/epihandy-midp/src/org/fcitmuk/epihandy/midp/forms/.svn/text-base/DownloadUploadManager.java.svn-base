...
import org.fcitmuk.epihandy.midp.transport.FormDownload;
...
public class DownloadUploadManager implements TransportLayerListener,AlertMessageListener {
	...
	/** Current alert is for download/editing of data */
	private static final byte CA_DATA_DOWNLOAD = 9;
	...
	private int currentStudyDefId;
	private int currentFormDefId;
	...
	public DownloadUploadManager(TransportLayer transportLayer,EpihandyController controller, String title,TransportLayerListener transportLayerListener, boolean showAlert) {
	...
	public TransportLayerListener getTransportLayerListener() {
		return this.transportLayerListener;
	}
	...
	public void downloadData(Displayable currentScreen, int studyDefId, int formDefId, String sessionReference, String userName, String password) {
		this.userName = userName;
		this.password = password;
		this.currentStudyDefId = studyDefId;
		this.currentFormDefId = formDefId;
		this.currentSessionRef = sessionReference;
		
		if (currentScreen != null)
			alertMsg.setPrevScreen(currentScreen);

		currentAction = CA_DATA_DOWNLOAD;
		alertMsg.showConfirm(MenuText.DOWNLOAD_DATA_PROMPT()+" "+sessionReference+"?");
	}
	...
	private void uploadData() {
		...
		if (!controller.getModel().storedDataExists()) {
			...
			if (currentAction == CA_SINGLE_DATA_UPLOAD) {
				if (currentFormData.isCompleted()) {
					...
					alertMsg.show(MenuText.FORM_SAVE_INCOMPLETE());
					currentAction = CA_NONE;
				}
			} else {
				if (controller.getModel().isComplete()) {
					...
					alertMsg.show(MenuText.FORM_SAVE_MULTIPLE_INCOMPLETE() + "\n\n" + controller.getModel().getInComplete());
					currentAction = CA_NONE;
				}
			}
		}
	...
	private void downloadData() {
		alertMsg.showProgress(MenuText.DOWNLOAD_DATA(), MenuText.DOWNLOADING_DATA());

		requestHeader.setLocale(LanguageSettings.getLocale());
		requestHeader.setAction(RequestHeader.ACTION_DOWNLOAD_DATA);

		Persistent[] inParams = new Persistent[] {
				new PersistentInt(currentFormDefId),
				new PersistentString(currentSessionRef)
		};
		
		setCommunicationParams();
		transportLayer.download(requestHeader, new PersistentArray(inParams), responseHeader, 
				controller.getFormDownload(currentStudyDefId, currentFormDefId), this, userName, password);
	}
	...
	public void setCommunicationParams() {
		...
		case CA_DATA_DOWNLOAD:
			url = settings.getSetting(TransportLayer.KEY_HTTP_URL);
			break;
		case CA_DATA_UPLOAD: 
	...
	public void updateCommunicationParams() {
		...
		case RequestHeader.ACTION_UPLOAD_DATA:
			...
			break;
		case RequestHeader.ACTION_DOWNLOAD_DATA:
			currentAction = CA_DATA_DOWNLOAD;
			break;
	...
	public void downloaded(Persistent dataInParams, Persistent dataIn, Persistent dataOutParams, Persistent dataOut) {
		...
		try {
			...
			} else if (currentAction == CA_DATA_DOWNLOAD) {
				FormDownload downloadData = (FormDownload)dataOut;
				FormData data = downloadData.getDownloadedFormData();
				if (data != null) {
					message = MenuText.DATA_DOWNLOAD_SAVED();
				}
			}
	...
	public void uploaded(Persistent dataInParams, Persistent dataIn, Persistent dataOutParams, Persistent dataOut) {
		...
		if (currentAction == CA_DATA_UPLOAD) {
			try {
				message = handleBatchDataUpload(dataInParams, dataIn, dataOutParams, dataOut, status);
			} catch (Exception e) {
				e.printStackTrace();
				message = MenuText.PROBLEM_CLEANING_STORE();
			}
		} else if (currentAction == CA_SINGLE_DATA_UPLOAD) {
			message = handleSingleDataUpload(dataIn, dataOut, status);
		} else
	...
	private String handleSingleDataUpload(Persistent dataIn, Persistent dataOut, ResponseHeader status) {
		...
		if (status.isSuccess()) {
			...
			if (GeneralSettings.deleteDataAfterUpload()) {
				controller.getModel().deleteFormData(controller.getCurrentStudy().getId(), formData);
			}
			
			StringBuffer uploadMessage = new StringBuffer();
			uploadMessage.append(MenuText.DATA_UPLOAD_SUCCESS());
			FormDataSummary[] summaries = uploadResponse.getUploadFormDataSummary();
			if (summaries.length > 0) {
				
				formData.buildDataDescription();
				String dataDescription = buildDataDescription(formData);
				
				uploadMessage.append("\n");
				uploadMessage.append(MenuText.UPLOAD_REFERENCE());
				uploadMessage.append(":");
				uploadMessage.append("\n'");
				uploadMessage.append(dataDescription);
				uploadMessage.append("' = ");
				uploadMessage.append(summaries[0].getReference());

				summaries[0].setDataDescription(dataDescription);

				controller.saveSessionReferences(summaries);
			}
	...
	private String handleBatchDataUpload(Persistent dataInParams, Persistent dataIn,
			Persistent dataOutParams, Persistent dataOut, ResponseHeader status) {
		...
		if (status.isSuccess()) {
			...
			StringBuffer sessionReferences = createSessionReferencesAfterMultipleFormUpload(formUpload, uploadResponse);

			removeSuccessfullyUploadedData(formUpload, uploadResponse, uploadMessage);
	...
	private String buildDataDescription(FormData formData) {
		FormDef form = formData.getDef();
		String desc = form.getName() + " " + formData.getDataDescription();
		return desc;
	}

	private StringBuffer createSessionReferencesAfterMultipleFormUpload(
			FormUpload formUpload, UploadResponse uploadResponse) {
		...
		if (uploadResponse.getSuccessCount() > 0) {
			...
			sessionReferences.append(MenuText.UPLOAD_REFERENCES());
			sessionReferences.append(":");
			FormDataSummary[] summaries = uploadResponse
					.getUploadFormDataSummary();
			for (int i = 0, n = summaries.length; i < n; i++) {

				FormDataSummary summary = summaries[i];
				sessionReferences.append("\n'");

				String description = formUpload.getDataDescription(
						summary.getStudyIndex(), summary.getFormIndex());
				sessionReferences.append(description);
				sessionReferences.append("' = ");
				sessionReferences.append(summary.getReference());

				summary.setDataDescription(description);
			}
			controller.saveSessionReferences(summaries);
		}
	...
	private void removeSuccessfullyUploadedData(FormUpload formUpload,
			UploadResponse uploadResponse, StringBuffer uploadMessage) {
		// Remove the successfully uploaded forms, if settings allow
		if (GeneralSettings.deleteDataAfterUpload()) {
			int deletedCount = deleteSuccessfulForms(formUpload, uploadResponse);
	...
	public void errorOccured(String errorMessage, Exception e) {
		...
		if(this.showAlert)			
			...
			this.transportLayerListener.errorOccured(errorMessage, e);
	}
