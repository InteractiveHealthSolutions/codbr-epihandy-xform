...
import org.fcitmuk.epihandy.midp.db.FormDataSummaryStore;
...
public class EpihandyController implements Controller, StorageListener, AlertMessageListener,TypeEditorListener, TransportLayerListener {
	...
	private FormDataSummaryStore formDataSummaryStore;

	public EpihandyController() {
		studyStore = new StudyStore();
		dataStore = new FormDataStore();
		formDataSummaryStore = new FormDataSummaryStore();
		...
		sessionLogView = new SessionLogView();
	}
	...
	private void setDefaults(String title){
		...
		defaultTypeEditor.setController(this); 
		sessionLogView.setController(this);
		...
		defaultTypeEditor.setDisplay(display);
		sessionLogView.setDisplay(display);

		studyListViewer.setTitle(title);
		formDefListViewer.setTitle(title);
		formDataListViewer.setTitle(title);
		formViewer.setTitle(title);
		defaultTypeEditor.setTitle(title);
		sessionLogView.setTitle(title);
	}
	...
	public void startEdit(QuestionData currentQuestion,int pos, int count){
		...
		TypeEditor editor = getTypeEditor(currentQuestion.getDef().getType());
	...
	public TypeEditor getTypeEditor(byte type) {
		Byte typeB = new Byte(type);
		if (typeEditors.containsKey(typeB)) {
			return (TypeEditor) typeEditors.get(typeB);
		} else {
			return defaultTypeEditor;
		}
	}
	...
	public void downloadUsers(){
		downloadMgr.downloadUsers(prevScreen, userMgr.getUserName(), userMgr.getPassword());
	}
	...
	public void downloadData(final Displayable screen, final int selectedStudyDefId, final int selectedFormDefId) {
		Form frm = new Form(screen.getTitle());
		final TextField txtField = new TextField(MenuText.SESSION_REFERENCE_PROMPT(),"",25,TextField.NUMERIC);
		frm.append(txtField);
		...
		frm.setCommandListener(new CommandListener() {
			public void commandAction(Command command, Displayable displayable) {
				if (command == DefaultCommands.cmdOk) {
					String sessionReference = txtField.getString();
					downloadMgr.downloadData(screen, 
							selectedStudyDefId, selectedFormDefId, sessionReference, 
	...
	public void logout(final Displayable prevScreen) {
		AlertMessage alertMsg = new AlertMessage(display, display.getCurrent().getTitle(), prevScreen, new AlertMessageListener() {
		...
		alertMsg.showConfirm(MenuText.LOGOUT_PROMPT());
	}
	...
	public FormDownload getFormDownload(int studyId, int formDefId) {
		return new FormDownload(model, studyId, formDefId);
	}
	
	public void loadSessionReferenceLog() {
		if(formDataSummaryStore.hasStoreData()){
			Vector sessionReferences = formDataSummaryStore.getSessionReferences();
			sessionLogView.showSessionLog(sessionReferences);
		}
		else{
			alertMsg.show(MenuText.NO_UPLOAD_REFERENCES());
		}
	}

	public void saveSessionReferences(FormDataSummary[] summaries) {
		for(int i=0; i < summaries.length; i++){
			formDataSummaryStore.saveSessionReferences(summaries[i]);
		}
	}
	
	public void clearSessionReferences() {
		formDataSummaryStore.clearSessionReferences();
	}
}
