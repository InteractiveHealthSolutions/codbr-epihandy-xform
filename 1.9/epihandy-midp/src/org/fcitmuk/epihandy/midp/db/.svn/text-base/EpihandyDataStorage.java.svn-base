...
public class EpihandyDataStorage {
	...
	public static void deleteUser(int userId, String username){
		Storage store = StorageFactory.getStorage(IDENTIFIER_USER_STORAGE,storageListener);

		UserList currentUsers = getUsers();
		Vector currentUsersList = currentUsers.getUsers();
		User user;
		for(int i=0;i<currentUsersList.size();i++){
			user=currentUsers.getUser(i);			
			if(user.getUserId()==userId && user.getName().equals(username)){
				currentUsersList.removeElementAt(i);
			}
		}
		store.delete();
		store.addNew(currentUsers);
	}
