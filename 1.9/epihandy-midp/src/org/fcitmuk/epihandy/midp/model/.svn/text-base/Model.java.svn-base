...
public class Model {
	...
	public boolean isComplete() {
		for (int i=0; i<formDataIds.size(); i++) {
			int recId = ((Integer) formDataIds.elementAt(i)).intValue();
			if (!dataStore.getFormData(getSelectedStudyDef().getId(), getSelectedFormDef().getId(), recId).isCompleted())
				return false;
		}
	...
	public String getInComplete() {
		String incompleteForms = "";
		for (int i=0; i<formDataIds.size(); i++) {
			int recId = ((Integer) formDataIds.elementAt(i)).intValue();
			if (!dataStore.getFormData(getSelectedStudyDef().getId(), getSelectedFormDef().getId(), recId).isCompleted()) {
				incompleteForms += (getSelectedStudyDef().getName() + ": " + getSelectedFormDef().getName() + "\n"); 
			}
		}
