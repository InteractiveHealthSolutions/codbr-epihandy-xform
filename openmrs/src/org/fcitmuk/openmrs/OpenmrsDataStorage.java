...
public class OpenmrsDataStorage {
	...
	/** The unique identifier for storage of patients. */
	private static final String XFORM_USER_STORAGE_NAME = "XformUser";
	
	private static final String XFORM_STORAGE_NAME = "Xform";

	private static final String XFORM_XML_STORAGE_NAME = "XformXml";
	...
	/**
	 * Method to delete all patients in the store
	 */
	...
	public static void saveXformUser(User xformUser){
		StorageFactory.getStorage(XFORM_USER_STORAGE_NAME,storageListener).addNew(xformUser);
	}
	
	/**
	 * Get a list of xformUsers
	 * @return
	 */
	public static Vector getXformUsers(){
		return StorageFactory.getStorage(XFORM_USER_STORAGE_NAME,storageListener).read(new User().getClass());
	}
	
	/**
	 * Get an xformUser with given username
	 * @param username must be a valid, non-empty string
	 * @return
	 */
	public static User getXformUser(String username){
		Vector xformUsers = StorageFactory.getStorage(XFORM_USER_STORAGE_NAME, storageListener).read(new User().getClass());
		for(int i=0; i<xformUsers.size(); i++){
			User xfu = ((User)xformUsers.elementAt(i));
			if(username.compareTo(xfu.getName()) == 0){
				return xfu;
			}
		}
	...
	public static Patient getPatient(int patientId){
		...
		if(patients != null){
			for(int i=0; i<patients.size(); i++){
				...
				if(p.getPatientId().intValue() == patientId)
					return p;//should only and only be one
			}
		}
	...
	public static Vector getXformsList(){
		Storage store = StorageFactory.getStorage(XFORM_STORAGE_NAME,storageListener);
		return store.read(new Xform().getClass()); 
	}
	
	public static XformXml getXformXml(int formId){
		Storage store = StorageFactory.getStorage(XFORM_XML_STORAGE_NAME,storageListener);
		Vector xml = store.read(new XformXml().getClass());
		for (int i = 0; i < xml.size(); i++) {
			XformXml x = (XformXml) xml.elementAt(i);
	...
	//TODO must check if there is any unsaved form in rec-store
	public static boolean deleteAllXforms()
	{
		return StorageFactory.getStorage(XFORM_STORAGE_NAME,storageListener).delete() 
				&& StorageFactory.getStorage(XFORM_XML_STORAGE_NAME, storageListener).delete();
	}
	
	public static void saveXform(Xform xform, XformXml formXml){
		StorageFactory.getStorage(XFORM_STORAGE_NAME,storageListener).addNew(xform);
		StorageFactory.getStorage(XFORM_XML_STORAGE_NAME, storageListener).addNew(formXml);
	}
	
	public static PatientList getPatientList(){
		Storage store = StorageFactory.getStorage(PATIENT_STORAGE_NAME,storageListener);
		Vector vect = store.read(new Patient().getClass()); 
		if(vect != null && vect.size() > 0)
			return new PatientList(vect);
		else return new PatientList();
	}
	
/**
 * Deletes all cohorts stored on the mobile
 * @author Omar Ahmed
 * @return - boolean notifying the delete was successful or not
 * date: 30-9-10
 */
	public static boolean deleteAllCohorts()
	{
		CohortList cohortList = null;		
		try {
			Storage store = StorageFactory.getStorage(COHORT_STORAGE_NAME,storageListener);
	...
	public static void deleteCohort(int cohortId){
		CohortList cohorts = getCohorts();
		if(cohorts == null || cohorts.size() == 0)
			return;

		Storage	store = StorageFactory.getStorage(COHORT_STORAGE_NAME,storageListener);
		Cohort coh = cohorts.getCohort(cohortId);
		store.delete(coh.getId());
	}
