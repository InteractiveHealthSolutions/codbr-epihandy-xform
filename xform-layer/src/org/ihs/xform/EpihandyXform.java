...
public class EpihandyXform{
		...
		if(true/*qtnData.getDef().isVisible() && qtnData.getDef().isEnabled()*/){		
			...
			if (value != null) {
				...
				if(qtnData.getDef().getType() != QuestionDef.QTN_TYPE_REPEAT){
					xpath = new String(xpath.toCharArray(), 1, xpath.length()-1);// TODO MAIMOONA .intern();
					//int pos = xpath.lastIndexOf('@'); String attributeName = null; //TODO MAIMOONA
	...
	private static void updateRepeatModel(Element instanceNode, QuestionData qtnData, FormDef formDef){
		...
		boolean nested = xpath.substring(formVarName.length()).indexOf("/") > -1;
	...
	public static void removeTextNode(Element node){
		for(int i=0; i<node.getChildCount(); i++){
			if(node.getType(i) == Element.TEXT){
				String text = node.getText(i).trim();
				if(text.length() > 0 && !(text.toLowerCase().compareTo("\n")==0)){
					node.removeChild(i);
					return;
				}
			}
	...
	public static String fromNode2String(Node node){
		...
		try{
			...
			serializer.flush();
			return new String(bos.toByteArray(), "UTF-8");// TODO MAIMOONA.intern();
		}
		catch(Exception e){
			e.printStackTrace();
			return null;
	...
	public static FormDef fromXform2FormDef(Document doc){
		return getFormDef(doc);
	}
	...
	public static Hashtable xml2Hashtable (InputStreamReader isr) throws IOException, XmlPullParserException
	{
		KXmlParser parser = new KXmlParser ();
		...
		try
		{
			...
			model = new Hashtable ();
			model = getNodeValuePairs (parser, model);
	...
	private static Hashtable getNodeValuePairs (KXmlParser parser, Hashtable model) throws XmlPullParserException, IOException
	{
		// Parse our XML file
		String name = "";
		String text = "";
	...
	public static Document getDocument(DataInputStream ipStream){
		Document doc = new Document();

		try{
			InputStreamReader xmlReader = new InputStreamReader(ipStream);//TODO MAIMOONA
	...
	private static void setAttributeDefaultValue(QuestionDef qtn, String variableName,Element dataNode,FormDef formDef){
		...
		if(xpath.startsWith(formDef.getVariableName() + "/"))
			xpath = StringUtils.replace(formDef.getVariableName() + "/", "", xpath);
	...
	public static FormData fromXform2FormData(Reader reader){			
		...
		Element dataNode = doc.getRootElement();
		Element datanode1 = dataNode.getElement(0);
		Element dataNode2 = dataNode.getElement(1)/*.getElement(0).getElement(0)*/;
		...
		for (int i = 0; i < numOfEntries; i++) {
			if (!element.isText(i) && element.getType(i) == Element.ELEMENT) {
				...
				if(tagname.equals("submit"))
					...
					if(!(parentName.toLowerCase().compareTo("group")==0)){
					...
					if(formDef.getPages() == null){
						...
						qtn.setId((byte)++currentQuestionNo);
					}
					else{
					...
					//qtn.setVariableName(child.getAttributeValue(null, "id")); //TODO MAIMOONA
					...
					String parentName = ((Element)child.getParent()).getName();
					if(StringUtils.equalsIgnoreCase(parentName,"input") 
							|| StringUtils.equalsIgnoreCase(parentName,"upload") 
							|| StringUtils.equalsIgnoreCase(parentName,"select") 
							|| StringUtils.equalsIgnoreCase(parentName,"select1") 
							|| StringUtils.equalsIgnoreCase(parentName,"item")){
						label = getTextContent(child);
					}
					else if(StringUtils.equalsIgnoreCase(parentName,"repeat")){
						if(questionDef != null && child.getChildCount() > 0 && child.isText(0))
							questionDef.setText(getTextContent(child));
					}
					else if(StringUtils.equalsIgnoreCase(parentName,"group")){
						label = getTextContent(child);
					}
				}
	...
	private static void parseDynamicOptions(DynamicOptionDef dynamicOptionDef, QuestionDef questionDef, QuestionDef parentQuestionDef, Node node, OptionDef optionDef, Hashtable parentOptionIdMap, FormDef formDef){
		...
		for(int index = 0; index < count; index++){
			...
			if(name.equals("item")){
				...
				optionList.addElement(optionDef);
	...
	private static OptionDef getOptionWithValue(DynamicOptionDef dynamicOptionDef, String value){
		...
		Enumeration iterator = dynamicOptionDef.getParentToChildOptions().keys();
		while(iterator.hasMoreElements()){
			OptionDef optionDef = getOptionWithValue((Vector)iterator.nextElement(),value);
			if(optionDef != null)
				return optionDef;
		}
		return null;
	...
	private static DynamicOptionDef getChildDynamicOptions(FormDef formDef, short questionId){
		...
		Enumeration iterator = formDef.getDynamicOptions().keys();
		while(iterator.hasMoreElements()){
			Object entry = iterator.nextElement();
			DynamicOptionDef dynamicOptionDef = (DynamicOptionDef)entry;
	...
	private static Element getInstanceNode(Element modelNode, String instanceId){
		...
		Vector result = xpls.getResult();
		if(result.size() > 0)
			return (Element)result.elementAt(0);
		return null;
	}
	...
	private static boolean addRepeatChildQtn(QuestionDef qtn, Vector repeats,Element child,Hashtable map,Hashtable rptKidmap){
		for(int i=0; i<repeats.size(); i++){
			QuestionDef rptQtn = (QuestionDef)repeats.elementAt(i);
			if(qtn.getVariableName().indexOf(rptQtn.getVariableName())>-1){
		...
		if(type != null){
			if(type.equals("xsd:string") || type.indexOf("string") != -1 ){
				...
				if("gps".equals(format))
					...
					def.setType(QuestionDef.QTN_TYPE_TEXT);//TODO MAIMOONA QTN_TYPE_PHONENUMBER);
				else
	...
	private static byte getAction(Object actn){
		...
		byte action = 0;
		if(StringUtils.equalsIgnoreCase(value,ATTRIBUTE_VALUE_ENABLE))
			action |= EpihandyConstants.ACTION_ENABLE;
		else if(StringUtils.equalsIgnoreCase(value,ATTRIBUTE_VALUE_DISABLE))
			action |= EpihandyConstants.ACTION_DISABLE;
		else if(StringUtils.equalsIgnoreCase(value,ATTRIBUTE_VALUE_SHOW))
			action |= EpihandyConstants.ACTION_SHOW;
		else if(StringUtils.equalsIgnoreCase(value,ATTRIBUTE_VALUE_HIDE))
			action |= EpihandyConstants.ACTION_HIDE;

		if(required != null && StringUtils.equalsIgnoreCase(required,"true()"))
	...
	private static SkipRule buildSkipRule(FormDef formDef, short questionId, String relevant, short skipRuleId, byte action){
		...
		skipRule.setId((byte) skipRuleId);
		...
		actionTargets.addElement(new Short(questionId));
	...
	private static Condition getSkipRuleCondition(FormDef formDef, String relevant, short id, byte action){		
		...
		if(!(value.equals("null") || value.equals(""))){
			...
			condition.setOperator((byte) EpihandyConstants.OPERATOR_NOT_IN_LIST);//TODO MAIMOONA OPERATOR_IS_NOT_NULL); //must be != ''
		else
	...
	private static Condition getValidationRuleCondition(FormDef formDef, String constraint, short questionId){		
		...
		QuestionDef questionDef = formDef.getQuestion((byte)questionId);
		if(questionDef == null)
			return null;
		...
		if(constraint.indexOf("length(.)")>-1 || constraint.indexOf("count(.)")>-1)
	...
	private static int getOperatorPos(String relevant){
		...
		int pos = StringUtils.lastIndexOf(relevant,"!=");
		if(pos > 0)
			return pos;

		pos = StringUtils.lastIndexOf(relevant,">=");
		if(pos > 0)
			return pos;
		...
		pos = StringUtils.lastIndexOf(relevant,"not(starts-with");
		if(pos > 0)
			return pos;

		pos = StringUtils.lastIndexOf(relevant,"starts-with");
		if(pos > 0)
			return pos;

		pos = StringUtils.lastIndexOf(relevant,"not(contains");
		if(pos > 0)
			return pos;

		pos = StringUtils.lastIndexOf(relevant,"contains");
		if(pos > 0)
			return pos;
	...
	private static int extractConditionsOperatorTokens(String relevant,int startPos, Vector list){
		...
		if(pos < 0){
			list.addElement(relevant.substring(startPos).trim());
			opSize = 0;
		}
		else
			list.addElement(relevant.substring(startPos,pos).trim());
