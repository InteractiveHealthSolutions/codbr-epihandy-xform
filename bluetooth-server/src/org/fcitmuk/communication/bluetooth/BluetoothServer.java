...
public final class BluetoothServer implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if(notifier == null){
            	raiseError("Can't open bluetooth url. NULL Notifier: ",null);
            	return;
            }
        ...
        while (!isClosed) {
            ...
            try {
            	//notifier = (StreamConnectionNotifier)Connector.open(url.toString());
            	System.out.println(name + " waiting for connections at address [" + SERVER_UUID +"] ...");
                conn = notifier.acceptAndOpen();
               
                System.out.println(name + " accepted connection at address [" + SERVER_UUID +"] ...");
            } catch (IOException e) {
                // wrong client or interrupted - continue anyway
            	//System.out.println(name + "[" + SERVER_UUID + "] IOException thrown (Possibly service stopped)");
                continue;
            }
            catch(Exception e){
            	e.printStackTrace();
