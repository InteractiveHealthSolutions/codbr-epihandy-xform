...
import org.fcitmuk.db.util.Serializer;
...
public class RMSStorage implements Storage{
	...
	/**
	 * Write the entire contents of this storage to the specified stream.
	 * 
	 * @param stream
	 * @throws IOException
	 * @throws RecordStoreException
	 */
	public Vector writeBytes(OutputStream stream) {
		Vector recordNums = new Vector();
		RecordEnumeration recEnum;
		if (open())
			try {
				recEnum = recStore.enumerateRecords(null, null, false);
				while (recEnum.hasNextElement()) {
					int recId = recEnum.nextRecordId();
					recordNums.addElement(new Integer(recId));
					byte[] recordBytes = recStore.getRecord(recId);
	...
	public Vector readIds() {
		...
		try {
			if (open()) {
				RecordEnumeration recs = recStore.enumerateRecords(null,null,false);
				int numRecs = recStore.getNumRecords();
				ids = new Vector(numRecs);
				while (recs.hasNextElement()) {
					int id = recs.nextRecordId();
