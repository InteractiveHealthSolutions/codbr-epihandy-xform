...
public class XPathLocationStep {
	...
	private void parseLocationStep(String locationStep) {
		...
		if (next.indexOf("::") == -1)
			if (next.startsWith("@")) {
				axis = "attribute";
				next = new String(next.toCharArray(), 1, next.length() - 1).intern();
			} else
				axis = "child";
		else {
			pattIndex = next.indexOf("::");
			if (pattIndex != -1) {
				axis = new String(next.toCharArray(), 0, pattIndex).intern();
		...
		pattIndex = next.indexOf("[");
		if (pattIndex != -1) {
			nodeTest = new String(next.toCharArray(), 0, pattIndex).intern();
			...
			// formed correctly
			predicate = new String(next.toCharArray(), 0, pattIndex).intern();
		} else
			nodeTest = next;

		// test for prefix
		if ((pattIndex = nodeTest.indexOf(":")) != -1) {
			nodePrefix = new String(nodeTest.toCharArray(), 0, pattIndex).intern();
	...
	public Vector getResult(Vector contextNodeSet) {
		...
		if (axis.equals("")) {
			if (nodeTest.equals("/")) {
				...
				if (startNode instanceof Element && !(startNode instanceof Document)) {
					Node tmp = null;
					while ((startNode instanceof Element) && (tmp = ((Element) startNode).getParent()) != null)
						startNode = tmp;
