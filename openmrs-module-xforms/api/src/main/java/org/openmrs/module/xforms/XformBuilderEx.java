...
public class XformBuilderEx {									
	...
	public static String buildXform(Form form) throws Exception {								
		...
		XformBuilder.parseTemplate(modelNode, formNode, formNode, bindings, groupNode, problemList, problemListItems, 0, DOMUtil.isNewPatientXformDoc(formNode));							
	...
	public static void simpleConcept(String token, Concept concept, String type, String fieldDescription, boolean required, Locale locale, FormField formField) {								
		addUInode(token, concept, type, fieldDescription, required, XformBuilder.CONTROL_INPUT, locale, getParentNode(formField, locale));							
	}								
									
	public static void numericConcept(String token, ConceptNumeric concept, String fieldDescription, boolean required, Locale locale, FormField formField) {								
		addUInode(token, concept, XformBuilder.DATA_TYPE_DECIMAL, fieldDescription, required, XformBuilder.CONTROL_INPUT, locale, getParentNode(formField, locale));							
	}								
									
	public static void selectSingle(String token, Concept concept,								
	                      			Collection<ConceptAnswer> answerList, String fieldDescription, boolean required,					
	                      			Locale locale, FormField formField) {					
									
		Element controlNode = addUInode(token, concept, XformBuilder.DATA_TYPE_TEXT, fieldDescription, required, XformBuilder.CONTROL_SELECT1, locale, getParentNode(formField, locale));							
		if (controlNode != null) {							
	...
	public static void selectMultiple(String token, Concept concept,								
	                        			Collection<ConceptAnswer> answerList, String fieldDescription, boolean required, Locale locale, FormField formField) {					
									
		Element controlNode = addUInode(token, concept, XformBuilder.DATA_TYPE_TEXT, fieldDescription, required, XformBuilder.CONTROL_SELECT, locale, getParentNode(formField, locale));							
		if (controlNode != null) {							
			addCodedUInodes(true, controlNode, answerList, concept, XformBuilder.DATA_TYPE_TEXT, XformBuilder.CONTROL_SELECT, locale);						
		}							
	}								
									
	public static void booleanConcept(String token, Concept concept, String fieldDescription, boolean required, Locale locale, FormField formField) {								
									
		Element controlNode = addUInode(token, concept, XformBuilder.DATA_TYPE_BOOLEAN, fieldDescription, required, XformBuilder.CONTROL_INPUT, locale, getParentNode(formField, locale));							
		/*if (controlNode != null) {							
	...
	private static Element addUInode(String token, Concept concept, String dataType, String fieldDescription, boolean required, String controlName, Locale locale, Element bodyNode){								
		...
		if(required){							
			bindNode.setAttribute(null, XformBuilder.ATTRIBUTE_REQUIRED, XformBuilder.XPATH_VALUE_TRUE);						
		}							
		...
		addHintNode(labelNode, concept, fieldDescription);							
	...
	private static Element getParentNode(FormField formField, Locale locale){								
		...
		if(formField.getParent() == null){							
			...
			addHintNode(labelNode, formField.getField().getConcept(), formField.getField().getDescription());						
	...
	public static void addProblemList(String token, Concept concept, boolean required,								
		...
		addHintNode(labelNode, concept, formField.getField().getDescription());							
	...
	private static void buildUInodes(Form form, TreeMap<Integer, TreeSet<FormField>> formStructure, Integer sectionId, Locale locale) {								
		...
		for(FormField formField : section){							
			...
			String fieldDescription = formField.getField().getDescription();
			if (field.getFieldType().getFieldTypeId().equals(						
				...
				if ( (name.contains("problem_added") || name.contains("problem_resolved")) &&					
					...
					booleanConcept(name, concept, fieldDescription, required, locale, formField);				
				}					
				else if (FormConstants.simpleDatatypes.containsKey(datatype.getHl7Abbreviation())){					
					simpleConcept(name, concept, XformBuilder.DATA_TYPE_TEXT, fieldDescription, required, locale, formField);				
				}					
					...
					numericConcept(name, conceptNumeric, fieldDescription, required, locale, formField);				
				} 					
					...
					if (formField.getMaxOccurs() != null && formField.getMaxOccurs().intValue() == -1) {				
						...
						if (field.getSelectMultiple()){			
							selectMultiple(name, concept, answers, fieldDescription, required, locale, formField);		
						}			
						else {			
							selectSingle(name, concept, answers, fieldDescription, required, locale, formField);		
						}			
					}				
				}					
				else if ("ED".equals(datatype.getHl7Abbreviation())){					
					simpleConcept(name, concept, XformBuilder.DATA_TYPE_BASE64BINARY, fieldDescription, required, locale, formField);				
				}					
			}//XFORM DOESNOT ASSIGN REQUIRED ATTRIBUTE TO ALL FIELDS HENCE TO CATER THIS MAKING IT HERE MAIMOONA						
			else if(field.getFieldType().getFieldTypeId().equals(						
					FormConstants.FIELD_TYPE_DATABASE)){				
				Element bindNode = (Element) bindings.get(name);					
				if (bindNode == null) {					
					System.out.println("NULL bindNode for: " + name);				
				}					
	...
	private static void addHintNode(Element labelNode, Concept concept, String fieldDescription) {								
		String hint = null;
		
		if(!com.mysql.jdbc.StringUtils.isEmptyOrWhitespaceOnly(fieldDescription)){
			hint = fieldDescription.trim();
		}
