...
public class XformsServer {
	...
	private void submitXforms(DataInputStream dis, DataOutputStream dos, String serializerKey) throws Exception {
		Map<String, Object> errorList = XformDataUploadManager.submitXforms(dis, new java.util.Date().toString(),serializerKey);
		// number of errors in submitted forms
		dos.writeByte(errorList.size());
		for (String key : errorList.keySet()) {
			//write identifier and error message associated...
			dos.writeUTF(key);
			dos.writeUTF((String) errorList.get(key));
		}
	}
