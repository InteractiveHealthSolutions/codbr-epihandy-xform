...
public class XformsQueueProcessor {
	...
	private Patient saveNewPatient(Element root, User creator, boolean propagateErrors, HttpServletRequest request) throws Exception{		
		...
		String valdd = DOMUtil.getElementValue(root,XformBuilder.NODE_DEATHDATE);
		if(valdd != null && valdd.length() > 0)
			try{ pt.setDeathDate(XformsUtil.fromSubmitString2Date(valdd)); } catch(Exception e){log.error(valdd,e); }

		pt.setDead("true".equals(DOMUtil.getElementValue(root, XformBuilder.NODE_PATIENT_DEAD)));
	...
	private void addPersonAttributes(Patient pt, Element root,XformsService xformsService, User creator) throws Exception{
		...
		for (PersonAttributeType type : personAttributeTypes) {
			NodeList nodes = root.getElementsByTagName("person_attribute."+type.getName());
	...
	private void addPersonAddresses(Patient pt, Element root, User creator) throws Exception{
		...
		addPersonAddressValue(XformBuilder.NODE_NAME_ADDRESS3, pa, root);
		addPersonAddressValue(XformBuilder.NODE_NAME_ADDRESS4, pa, root);
		addPersonAddressValue(XformBuilder.NODE_NAME_ADDRESS5, pa, root);
		addPersonAddressValue(XformBuilder.NODE_NAME_ADDRESS6, pa, root);
		...
		addPersonAddressValue(XformBuilder.NODE_NAME_PREFERRED, pa, root);
	...
	private void addPersonAddressValue(String name, PersonAddress pa, Element root) throws Exception{
		...
		else if(name.equals(XformBuilder.NODE_NAME_ADDRESS3))
			pa.setAddress3(value);
		else if(name.equals(XformBuilder.NODE_NAME_ADDRESS4))
			pa.setAddress4(value);
		else if(name.equals(XformBuilder.NODE_NAME_ADDRESS5))
			pa.setAddress5(value);
		else if(name.equals(XformBuilder.NODE_NAME_ADDRESS6))
		...
		else if(name.equals(XformBuilder.NODE_NAME_PREFERRED))
			pa.setPreferred(Boolean.parseBoolean(value));
	...
	private void addPersonAddressesUptoN(Patient pt, Element rootElement, User creator) throws Exception{
		//MAIMOONA assuming only 20 addresses could be given in a form , hence to allow skipping few numbers in address postfix
		for (int n = 1; n <= 20; n++) {
			NodeList addN = rootElement.getElementsByTagName(XformBuilder.NODE_NAME_PERSON_ADDRESS+n);
			// if no address with postfix n i.e. person_address1/2/3/4/5...n
			if(addN == null || addN.getLength() == 0){
				continue;
			}
			
			//Only 1 node is possible with a tagname
			Node addNode = addN.item(0);
			PersonAddress pa = new PersonAddress();
			pa.setCreator(creator);
			pa.setDateCreated(pt.getDateCreated());
			pa.setPreferred(false);

			NodeList addressElements = addNode.getChildNodes();
			for (int i = 0; i < addressElements.getLength(); i++) {
				if( addressElements.item(i).getNodeType() == Node.TEXT_NODE){
					continue;
				}
				String name = addressElements.item(i).getNodeName();
				String value = addressElements.item(i).getTextContent();
				if(value == null || value.length() == 0)
					continue;

				if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS1))
					pa.setAddress1(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS2))
					pa.setAddress2(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS3))
					pa.setAddress3(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS4))
					pa.setAddress4(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS5))
					pa.setAddress5(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_ADDRESS6))
					pa.setAddress6(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_CITY_VILLAGE))
					pa.setCityVillage(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_STATE_PROVINCE))
					pa.setStateProvince(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_POSTAL_CODE))
					pa.setPostalCode(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_COUNTRY))
					pa.setCountry(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_LATITUDE))
					pa.setLatitude(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_LONGITUDE))
					pa.setLongitude(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_COUNTY_DISTRICT))
					pa.setCountyDistrict(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_PREFERRED))
					pa.setPreferred(Boolean.parseBoolean(value));
				/*else if(name.endsWith(XformBuilder.NODE_NAME_NEIGHBORHOOD_CELL))
					pa.setNeighborhoodCell(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_REGION))
					pa.setRegion(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_SUBREGION))
					pa.setSubregion(value);
				else if(name.endsWith(XformBuilder.NODE_NAME_TOWNSHIP_DIVISION))
					pa.setTownshipDivision(value);*/
			}
	...
	private void fillPatientIdIfMissing(Document doc) throws Exception{
		//MAIMOONA if there is any tag reflecting new patient form, skip it and check if any patient already exists, if so, throw exception
		if(!DOMUtil.isNewPatientXformDoc(doc)){
			String patientid = DOMUtil.getElementValue(doc,XformBuilder.NODE_PATIENT_PATIENT_ID);
		...
		patient.setGender(gender==null?"NP":gender.trim());
		...
		String valdd = DOMUtil.getElementValue(doc,XformBuilder.NODE_PATIENT_DEATHDATE);
		if(valdd != null && valdd.length() > 0)
			try{ patient.setDeathDate(XformsUtil.fromSubmitString2Date(valdd)); } catch(Exception e){log.error(valdd,e); }

		patient.setDead("true".equals(DOMUtil.getElementValue(doc, XformBuilder.NODE_PATIENT_PATIENT_DEAD)));
		...
		// MAIMOONA If multiple addresses are given by sequence numbers
		addPersonAddressesUptoN(patient, doc.getDocumentElement(), patient.getCreator());
		
		Context.getPatientService().savePatient(patient);
		
		// MAIMOONA If new patient xform create patientId element is doc to be used by queue processor further..
		if(DOMUtil.isNewPatientXformDoc(doc)){
			Element patientIdElement = doc.createElement(XformBuilder.NODE_PATIENT_PATIENT_ID);
			patientIdElement.setTextContent(patient.getPatientId().toString());
			doc.getElementsByTagName("patient").item(0).appendChild(patientIdElement);
		}
		else {
