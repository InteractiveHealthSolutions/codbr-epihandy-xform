...
public class XformDataUploadManager {
	 ...
	 * @throws Exception
	 */
	public static Map<String, Object> submitXforms(InputStream is, String sessionId, String serializerKey) throws Exception {
		...
		Map<String, Object> successList = new HashMap<String, Object>();

		for (Document doc : docs)
		{
			String patientIdentifier = XformsQueueProcessor.getPatientIdentifier(doc);
			String patientid = DOMUtil.getElementValue(doc,XformBuilder.NODE_PATIENT_PATIENT_ID);

			
			
			try {
				queueForm(XformsUtil.doc2String(doc), true, null);
				
				if(!StringUtils.isEmptyOrWhitespaceOnly(patientIdentifier)){
					successList.put(patientIdentifier, "SUCCESS");
				}
				else {
					successList.put(patientid, "SUCCESS");
				}
			} 
			catch (Exception e) {
				e.printStackTrace();
				...
				if(!StringUtils.isEmptyOrWhitespaceOnly(patientIdentifier)){
					errorList.put(patientIdentifier, e.getMessage());
				}
				else {
					errorList.put(patientid, e.getMessage());
				}
			}
		}
		
		//merging all responses first success and then error, so that if there is any ID with errors, it is flagged with caution
		
		Map<String, Object> responseList = new HashMap<String, Object>();
		responseList.putAll(successList);
		responseList.putAll(errorList);
