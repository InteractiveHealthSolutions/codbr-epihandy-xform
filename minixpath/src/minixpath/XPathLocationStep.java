	...
	public Vector getResult(Vector contextNodeSet) {
		...
		if (axis.equals("")) {
			if (nodeTest.equals("/")) {
				...
				if (startNode instanceof Element) {
					Element tmp = null;
					while ((((Element) startNode).getParent() instanceof Element) && // MAIMOONA : TO Prevent classCastException on reaching Document
					(tmp = (Element)((Element) startNode).getParent()) != null)
						startNode = tmp;
