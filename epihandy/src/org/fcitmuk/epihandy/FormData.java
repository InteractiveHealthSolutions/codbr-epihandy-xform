...
public class FormData  extends AbstractRecord{
	...
	/**
	 * This is the numeric identifier for the form data object used when form data has
	 * been assigned an identifier on the server (for example for updates/download data)
	 */
	private int dataId = EpihandyConstants.NULL_ID;
	...
	private Hashtable autocalculatedValues = new Hashtable();
	
	public Hashtable getAutocalculatedValues() {
		return autocalculatedValues;
	}

	public void setAutocalculatedValues(Hashtable autocalculatedValues) {
		this.autocalculatedValues = autocalculatedValues;
	}
	...
	public FormData(FormData data){
		...
		setCompleted(true);
		setAutocalculatedValues(data.getAutocalculatedValues());
		//MAIMOONA if question type is Barcode & question default value is $*{autocalculate}*$
		fillAutocalculatedValues();
	}
		...
		for(int i=0; i<this.getDef().getPages().size(); i++){
			...
			if (pageDef.getQuestions() != null) {
				for(int j=0; j<pageDef.getQuestions().size(); j++){
					...
					if(qtnDef.getText() == null || qtnDef.getText().trim().equals("")){
						qtnDef.setVisible(false);
					}
				}
			}
	...
	//MAIMOONA if question type is Barcode & question default value is $*{autocalculate}*$
	public void fillAutocalculatedValues(){
		String autocalctag = "$*{autocalculated}*$";
		for(byte i=0; i<this.getPages().size(); i++){
			...
			for(short j=0; j<pageData.getQuestions().size(); j++){
				...
				if(qtnData.getDef().getDefaultValue() != null){
					String defVal = qtnData.getDef().getDefaultValue().trim();
					// we just want to override Question which have not been filled yet. otherwise already answered should be persisted
					if(defVal.toLowerCase().indexOf(autocalctag) != -1){
						if(qtnData.getAnswer() == null 
								|| qtnData.getAnswer().toString().trim().equals("")
								|| qtnData.getAnswer().toString().toLowerCase().indexOf(autocalctag) != -1){
							Object val = autocalculatedValues.get(qtnData.getDef().getVariableName());
							if(val != null) qtnData.setAnswer(val);
						}
					}
				}
			}	
	...
	// MAIMOONA web xforms` calculation field doenot work hence a hack to allow default value field to be used as a flag to copy value of some other question
	public void setCalculationDefault(){
		for(byte i=0; i<this.getPages().size(); i++){
			PageData pageData = (PageData)this.getPages().elementAt(i);
			for(short j=0; j<pageData.getQuestions().size(); j++){
				QuestionData qtnData = (QuestionData)pageData.getQuestions().elementAt(j);
				if(qtnData.getDef().getDefaultValue() != null){
					...
					if(defVal.startsWith("${") && defVal.endsWith("}$")){
						String fieldName = defVal.substring(defVal.indexOf("${")+2, defVal.indexOf("}$"));
						QuestionData qa = getQuestion(fieldName);
						String val = qa.getValueAnswer();
						if(val != null){
							qtnData.setTextAnswer(val);
						}
					}
				}
			}	
	...
	public QuestionData getQuestionMatchingEnd(String varName){
		for(int i=0; i<this.getDef().getPages().size(); i++){
			PageDef page = (PageDef)this.getDef().getPages().elementAt(i);
			for(int j=0; j<page.getQuestions().size(); j++){
				QuestionDef qtn = (QuestionDef)page.getQuestions().elementAt(j);
				if(qtn.getVariableName().endsWith(varName))
					return getQuestion(qtn.getId());
			}
		}
	...
	public void read(DataInputStream dis) throws IOException, InstantiationException, IllegalAccessException {
		setRecordId(dis.readInt());
		setDataId(dis.readInt());
		setDefId(dis.readInt());
		setCompleted(dis.readBoolean());
		setPages(PersistentHelper.readMedium(dis,PageData.class));
	}

	public void write(DataOutputStream dos) throws IOException {
		dos.writeInt(getRecordId());
		dos.writeInt(getDataId());
