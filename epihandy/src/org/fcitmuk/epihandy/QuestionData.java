...
public class QuestionData implements Persistent{
	...
	public void setTextAnswer(String textAnswer){
		...
		case QuestionDef.QTN_TYPE_DATE:
			setDateAnswer(textAnswer, dateSubmitFormat);
			break;
		case QuestionDef.QTN_TYPE_DATE_TIME:
			setDateTimeAnswer(textAnswer, dateSubmitFormat);
			break;
		case QuestionDef.QTN_TYPE_TIME:
			setTimeAnswer(textAnswer);
			break;
	...
	public void setDateAnswer(String dateAnswer, int format) {
		boolean isFunction = isDateFunction(dateAnswer);
		if (dateAnswer == null || dateAnswer.trim().equals("")
				|| (dateAnswer.indexOf("-") < 0 && !isFunction)) {
			return;
		} else if (isFunction) {
			answer = new Date();
			return;
		}
		Calendar cal = Calendar.getInstance(EpihandyConstants.DEFAULT_TIME_ZONE);
		setDatePart(dateAnswer, format, cal);
		answer = cal.getTime();
	}
	
	public void setTimeAnswer(String timeAnswer) {
		// format hh:mm:ss AM/PM
		boolean isFunction = isDateFunction(timeAnswer);
		if (timeAnswer == null || timeAnswer.trim().equals("")
				|| (timeAnswer.indexOf(":") < 0 && !isFunction)) {
			return;
		} else if (isFunction) {
			answer = new Date();
			return;
		...
		Calendar cal = Calendar.getInstance(EpihandyConstants.DEFAULT_TIME_ZONE);
		setTimePart(timeAnswer, cal);
		answer = cal.getTime();
	}
	
	public void setDateTimeAnswer(String dateTimeAnswer, int dateFormat) {
		boolean isFunction = isDateFunction(dateTimeAnswer);
		if (dateTimeAnswer == null
				|| dateTimeAnswer.trim().equals("")
				|| ((dateTimeAnswer.indexOf(":") < 0 || dateTimeAnswer
						.indexOf("-") < 0) && !isFunction)) {
			return;
		} else if (isFunction) {
			answer = new Date();
			return;
		...
		Calendar cal = Calendar.getInstance(EpihandyConstants.DEFAULT_TIME_ZONE);
		String datePart = dateTimeAnswer.substring(0, 9);
		String timePart = dateTimeAnswer.substring(10);
		setDatePart(datePart, dateFormat, cal);
		setTimePart(timePart, cal);
		answer = cal.getTime();
	}
	
	private void setDatePart(String dateAnswer, int format, Calendar cal) {
		// format usually dd-mm-yyyy or yyyyy-mm-dd or mm-dd-yyyy
		String first = dateAnswer.substring(0, dateAnswer.indexOf("-"));
		String second = dateAnswer.substring(first.length()+1, dateAnswer.indexOf("-", first.length()+1));
		String third = dateAnswer.substring(first.length()+1+second.length()+1);
		if (format == 0) { // dd-mm-yyyy
			cal.set(Calendar.DAY_OF_MONTH, Integer.parseInt(first));
			cal.set(Calendar.MONTH, Integer.parseInt(second)-1);
			cal.set(Calendar.YEAR, Integer.parseInt(third));
		} else if (format == 1) { // mm-dd-yyyy
			cal.set(Calendar.DAY_OF_MONTH, Integer.parseInt(second));
			cal.set(Calendar.MONTH, Integer.parseInt(first)-1);
			cal.set(Calendar.YEAR, Integer.parseInt(third));
		} else { // yyyy-mm-dd
			cal.set(Calendar.DAY_OF_MONTH, Integer.parseInt(third));
			cal.set(Calendar.MONTH, Integer.parseInt(second)-1);
			cal.set(Calendar.YEAR, Integer.parseInt(first));
		}
	}
	
	private void setTimePart(String timeAnswer, Calendar cal) {
		String hour = timeAnswer.substring(0, 2);
		String minute = timeAnswer.substring(3, 5);
		String second = timeAnswer.substring(6, 8);
		String ampm = timeAnswer.substring(9, 11);
	...
	public void setOptionAnswers(Vector vals){
		...
		for(short j=0; j<vals.size(); j++){
			...
			for(short i=0; i<options.size(); i++){
				OptionDef option = (OptionDef)options.elementAt(i);
...
//		TODO This method should be refactored with the one above.
	...
	public String getText(){
		String helpText = (getDef().getHelpText()==null||getDef().getHelpText().trim().equals(""))?"":getDef().getHelpText().trim();
		//String val =  helpText==""?getDef().getText():(helpText+"\n["+getDef().getText().toLowerCase()+"]");
		String val=helpText;
	...
	private void writeAnswer(DataOutputStream dos) throws IOException {	
		...
		case QuestionDef.QTN_TYPE_LIST_EXCLUSIVE_DYNAMIC:
			if(getAnswer() != null){
				...
				dos.writeShort(Short.parseShort(getOptionAnswerIndices().toString()));
			}
			else
