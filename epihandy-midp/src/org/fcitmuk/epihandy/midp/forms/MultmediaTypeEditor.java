	...
	private void view(){
		...
		try{
			...
			if(questionData.getDef().getType() == QuestionDef.QTN_TYPE_IMAGE)
				...
				player = Manager.createPlayer(new ByteArrayInputStream(data), "audio/"+((EpihandyController)getController()).getFormManager().getFormSettings().getAudioFormat()); //audio/mpeg(mp3)
				player.start();
			}
			else if(questionData.getDef().getType() == QuestionDef.QTN_TYPE_VIDEO){
				player = Manager.createPlayer(new ByteArrayInputStream(data), "video/"+((EpihandyController)getController()).getFormManager().getFormSettings().getVideoFormat());
				player.realize();
...
//		creates a new player and set it to realize Manager.createPlayer("capture://audio"); 
	...
	private boolean saveImage(){
		...
		try {
			...
			byte[] image = ((VideoControl)control).getSnapshot(((EpihandyController)getController()).getFormManager().getFormSettings().getPictureParameters()); //"encoding=png&width=640&height=480"
