...
public class GPSTypeEditorImpl extends GPSTypeEditor implements AlertMessageListener, Runnable {
	...
	private void view(QuestionData questionData){
		try{
			...
			TextField txtField = new TextField("Latitude",latitude,20,TextField.UNEDITABLE);
			((Form)screen).append(txtField);
			
			txtField = new TextField("Longitude",longitude,20,TextField.UNEDITABLE);
			((Form)screen).append(txtField);

			txtField = new TextField("Altitude",altitude,20,TextField.UNEDITABLE);
			((Form)screen).append(txtField);
