...
public class FormView extends AbstractView implements AlertMessageListener {
	...
	public void onEndEdit(boolean saved, Command cmd){
		...
		if(cmd != DefaultCommands.cmdBackParent && getEpihandyController().getFormManager().getFormSettings().isSingleQtnEdit())
		{
	...
	public void showForm(FormData data,FormListener listener, boolean allowDelete){
		try{
			...
			EpihandyDataStorage.storageListener = new StorageListener() {
				public void errorOccured(String errorMessage, Exception e) {
					e.printStackTrace();
					display.setCurrent(new Alert("Error storing form : "+e.getMessage()), screen);
				}
			};
	...
	private void showPage(int pageIndex,Integer currentQuestionIndex){
		...
		boolean numbering = getEpihandyController().getFormManager().getFormSettings().isQuestionNumbering();
		...
		for(int i=0; i<qns.size(); i++){
			...
			if(qn.getDef().isVisible()){
				String s = "";
				if(qn.getDef().isLocked()) //Exclamation before
					s += "- ";
	...
	private boolean selectMissingValueQtn(byte pageNo){	
		...
		for(short i=0; i<displayedQuestions.size(); i++){
	...
	public void handleListSelectCommand(Displayable d){
		...
		currentQuestion = (QuestionData)displayedQuestions.elementAt(currentQuestionIndex);
		if(currentQuestion.getDef().isEnabled() && (currentQuestion.getDef().isLocked() == false))//allow only fields which are not locked and enabled
		{
